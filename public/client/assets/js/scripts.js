$(function() {
    /* encoded-54434312975 Range Slider */
    if($('#steps-slider').length) {
        ...
        slider.noUiSlider.on('update', function (values) {
            $('#encoded-54434312975-range-from').text(numFormat(Math.round(values[0])));
            $('#encoded-54434312975-range-to').text(numFormat(Math.round(values[1])));
        });
    }
    /* encoded-54434312975 Range Slider */
    ...
    function countDown(){
        ...
        var elTimeCounter = $('.encoded-54312311');
    ...
    if($('.featured-categories').length) {
        $('.featured-categories').click(function () {
            ...
            $('.encoded-5455432211').each(function () {
    ...
    if($('.most-sales-categories').length) {
        $('.most-sales-categories').click(function () {
            var category = $(this).data('val');
            $('.encoded-5443431223312').each(function () {
                if ($(this).hasClass(category))
                    $(this).fadeIn();
                else
                    $(this).fadeOut(0);
